...
class WBNetworkManager: AFHTTPSessionManager {
    ...
    func tokenRequest(method: WBHTTPMethod = .GET, URLString: String, parameters: [String: AnyObject]?, name: String? = nil, data: Data? = nil, completion: @escaping (_ json: AnyObject?, _ isSuccess: Bool)->()) {
        ...
        if let name = name, let data = data {
            // 上传文件
            upload(URLString: URLString, parameters: parameters, name: name, data: data, completion: completion as! (Any?, Bool) -> ())
        } else {
...
//        }
    ...
    func request(method: WBHTTPMethod = .GET, URLString: String, parameters: [String: AnyObject]?, completion: @escaping (_ json: AnyObject?, _ isSuccess: Bool)->()) {
        ...
        // 失败回调  ((URLSessionDataTask?, Error) -> Swift.Void)? = nil
//        func failure(task: URLSessionDataTask? , error: Error) {
...
//                        completion(nil, false)
//        }
        let failure: ((URLSessionDataTask?, Error)->())? = { (task, error)  in
...
// Cannot convert value of type '(URLSessionDataTask?, NSError) -> ()' to expected argument type '((URLSessionDataTask?, Error) -> Void)?'
        ...
        if method == .GET {
            ...
            post(URLString, parameters: parameters, progress: nil, success: success, failure: nil)
        }
